...
After startup JBM installs an eBPF program that adds kprobe for `finish_task_switch` function. By hooking `finish_task_switch` function, we can detect when a task become off-cpu and on-cpu, hence by computing duration since last off, we can detect if a task has blocked for a long time.
